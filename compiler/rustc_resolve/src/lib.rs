...
enum ResolutionError<'ra> {
    ...
    /// An item in `impl Sub for Ty` is ambiguous because multiple supertraits
    /// of `Sub` define an item with the same name.
    SupertraitItemAmbiguity {
        item_name: Ident,
...
pub struct Resolver<'ra, 'tcx> {
    ...
    /// Maps trait `DefId` to supertrait `DefId`s. Populated during trait definition resolution.
    trait_supertraits: FxHashMap<DefId, Vec<DefId>>,
    /// Synthetic supertrait impls to generate in lowering.
    /// Maps the original impl's `NodeId` to synthetic impls for each supertrait.
    ///
    /// NOTE: This is the first use of `create_def` during late resolution
    /// (normally defs are only created in `def_collector.rs` and
    /// `build_reduced_graph.rs`). We create them here because transparent
    /// supertrait item detection requires name resolution results.
    synthetic_supertrait_impls: NodeMap<Vec<ty::SyntheticSupertraitImpl>>,
    /// Maps synthetic supertrait impl DefId → original item DefIds.
    synthetic_impl_to_original_items: FxIndexMap<DefId, Vec<DefId>>,
...
impl<'ra, 'tcx> Resolver<'ra, 'tcx> {
    pub fn new(
        ...
        let mut resolver = Resolver {
            ...
            synthetic_supertrait_impls: Default::default(),
            ...
            synthetic_impl_to_original_items: Default::default(),
            features: tcx.features(),
            ..
        };
    ...
    pub fn into_outputs(self) -> ResolverOutputs<'tcx> {
        ...
        let global_ctxt = ResolverGlobalCtxt {
            ...
            synthetic_impl_to_original_items: self.synthetic_impl_to_original_items,
        };
        let ast_lowering = ty::ResolverAstLowering {
            ...
            disambiguators,
            synthetic_supertrait_impls: self.synthetic_supertrait_impls,
        };
