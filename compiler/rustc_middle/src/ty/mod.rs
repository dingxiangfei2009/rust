...
pub struct ImplTraitHeader<'tcx> {
    ...
    /// `true` for impls generated by `auto impl Super for trait Sub`.
    /// Used by coherence to allow manual impls to suppress auto impls.
    pub is_auto_impl: bool,
}
...
impl<'tcx> TyCtxt<'tcx> {
    ...
    pub fn impls_are_allowed_to_overlap(
        ...
        // Auto impls from `auto impl Super for trait Sub` are allowed to overlap
        // with any other impl. For auto + manual overlap, the manual impl takes
        // precedence (suppression). For auto + auto overlap, ambiguity is resolved
        // when a concrete type implements both subtraits — the user must write a
        // manual `impl Super for Foo = SubA::Super;` to disambiguate.
        if impl1.is_auto_impl || impl2.is_auto_impl {
            return Some(ImplOverlapKind::Permitted { marker: false });
        }
